...
let translate (globals, functions) =
  ...
  and list_t t   = L.struct_type context [| L.i32_type context; L.i32_type context; (L.pointer_type t) |]
  in
  let ptr_list_t t = L.pointer_type (list_t t) 
  in
  ...
  let rec ltype_of_typ = function
    ...
    | A.String -> str_t
    | A.Void  -> void_t
    | A.List t -> ptr_list_t (ltype_of_typ t)
  in
  ...
  let list_get : L.llvalue StringMap.t = 
    let list_get_ty m typ = 
       ...
       let def = L.define_function ("list_get" ^ defName) (L.function_type ltype [| ptr_list_t ltype; i32_t |]) the_module in
       ...
       let listArrayElementPtr = L.build_gep listArrayPtr [| idx |] "list_arry_element_ptr" build in
  ...
  let list_push : L.llvalue StringMap.t = 
    let list_push_ty m typ =
     ...
     let listArrayLoad = L.build_load listArrayPtr "list_array_load" build in
     (* listAllocSize = int* *)
     let listAllocSize = L.build_struct_gep listLoad 0 "list_alloc_size" build in
     (* listSizePtr = int* *)
  ...
  (* list** list_ptr_ptr *)
  let init_list builder list_ptr_ptr list_type = 
    (* list* *)
    let list_ptr = L.build_alloca (list_t list_type) "list_alloc" builder in
    (* list* list** *)
    ignore(L.build_store list_ptr list_ptr_ptr builder);
    (* initialize list *)
    let ptr = L.build_struct_gep list_ptr 0 "list.size" builder in 
       ignore(L.build_store (L.const_int i32_t 0) ptr builder);
    let ptr = L.build_struct_gep list_ptr 1 "list.size2" builder in 
       ignore(L.build_store (L.const_int i32_t 0) ptr builder);
    let ptr = L.build_struct_gep list_ptr 2 "list.arry" builder in 
     (* TODO: allocate nothing and have list grow dynamically *)
      let p = L.build_array_alloca (list_type) (L.const_int i32_t 1000) "p" builder in
  ...
  let build_function_body fdecl =
    ...
    ignore(
      match t with 
        A.List list_type -> init_list builder local (ltype_of_typ list_type)
      | _ -> ()
    );
  ...
  ignore(
    match t with 
      A.List list_type -> init_list builder local_var (ltype_of_typ list_type)
    | _ -> ()
  );
  StringMap.add n local_var m 
    ...
    in
    let list_id_to_ptr builder id = L.build_load (lookup id) id builder
    in
    ...
    | SListGet (list_type, id, e) ->
      L.build_call (StringMap.find (type_str list_type) list_get) [| list_id_to_ptr builder id; (expr builder e) |] "list_get" builder
    | SListSize (list_type, id) -> 
      L.build_call ((StringMap.find (type_str list_type)) list_size) [| list_id_to_ptr builder id |] "list_size" builder
    | SListPop (list_type, id) -> 
      L.build_call ((StringMap.find (type_str list_type)) list_pop) [| list_id_to_ptr builder id |] "list_pop" builder
    ...
    let rec stmt builder = function
      ...
      | SListPush (id, e) -> 
        ignore(L.build_call (StringMap.find (type_str (fst e)) list_push) [| list_id_to_ptr builder id; (expr builder e) |] "" builder); builder 
      | SListSet (list_type, id, e1, e2) ->
        ignore(L.build_call (StringMap.find (type_str list_type) list_set) [| list_id_to_ptr builder id; (expr builder e1); (expr builder e2) |] "" builder); builder
